...
def run():
    ...
    time.sleep(2)	# time for zebra to create api socket
    r1.cmd('/usr/sbin/ospfd -f %(workdir)s/r1ospfd.conf -d -z %(workdir)s/r1zebra.api -i %(workdir)s/r1ospfd.pid' % dirs)
    r2.cmd('/usr/sbin/ospfd -f %(workdir)s/r2ospfd.conf -d -z %(workdir)s/r2zebra.api -i %(workdir)s/r2ospfd.pid' % dirs)

    lb1=net.getNodeByName('lb1')
    lb2=net.getNodeByName('lb2')
    lb1.cmd('/usr/sbin/zebra -f %(workdir)s/lb1zebra.conf -d -z %(workdir)s/lb1zebra.api -i %(workdir)s/lb1zebra.pid' % dirs)
    time.sleep(2)	# time for zebra to create api socket
    lb2.cmd('/usr/sbin/zebra -f %(workdir)s/lb2zebra.conf -d -z %(workdir)s/lb2zebra.api -i %(workdir)s/lb2zebra.pid' % dirs)
    time.sleep(2)	# time for zebra to create api socket
    lb1.cmd('/usr/sbin/ospfd -f %(workdir)s/lb1ospfd.conf -d -z %(workdir)s/lb1zebra.api -i %(workdir)s/lb1ospfd.pid' % dirs)
    lb2.cmd('/usr/sbin/ospfd -f %(workdir)s/lb2ospfd.conf -d -z %(workdir)s/lb2zebra.api -i %(workdir)s/lb2ospfd.pid' % dirs)
    lb1.cmd('/usr/sbin/keepalived -n -f %(workdir)s/keep1.conf -p %(workdir)s/keep1.pid -r %(workdir)s/vrrp1.pid -c %(workdir)s/check1.pid &' % dirs)
    lb2.cmd('/usr/sbin/keepalived -n -f %(workdir)s/keep2.conf -p %(workdir)s/keep2.pid -r %(workdir)s/vrrp2.pid -c %(workdir)s/check2.pid &' % dirs)

    CLI( net )
    net.stop()
    os.system("killall -9 zebra ospfd keepalived")
    os.system("rm -f *api*")
    os.system("rm -f *pid*")
